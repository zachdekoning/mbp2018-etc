...
static int applesmc_create_accelerometer(struct applesmc_device *smc)
{
	...
	smc->idev = input_allocate_polled_device();
	if (!smc->idev) {
		ret = -ENOMEM;
		goto out_sysfs;
	}

	smc->idev->private = smc;
	smc->idev->poll = applesmc_idev_poll;
	smc->idev->poll_interval = APPLESMC_POLL_INTERVAL;
	...
	ret = input_register_polled_device(smc->idev);
	if (ret)
		goto out_idev;
